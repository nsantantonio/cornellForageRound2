...
\frame[label = always]{\frametitle{Mac Key 1970}
\begin{center}
...
\frame[label = always]{\frametitle{Evolution of allohexaploid wheat}
\begin{columns}
...
\frame[label = always]{\frametitle{Allohexaploid wheat}
\begin{columns}
...
\frame[label = always]{\frametitle{Hybrid generation}
  \begin{center}
...
\frame[label = always]{\frametitle{Allopolyploid formation}
  \begin{center}
\begin{tikzpicture}[node distance=2cm and 1cm, auto, draw = DarkGray]
  \node (invisT) at (1, 4) {};
  \node (invisB) at (1, 2) {};
...
\frame[label = always]{\frametitle{Cornell Winter Wheat Master Population}
\begin{columns}
\begin{column}{0.15\linewidth}
  \includegraphics[page=12, clip, trim = 0in 0in 8.5in 0in, height=0.85\textheight]{"\string~/Dropbox/CUpres/armCartoon2"}
\end{column}
...
\frame[label = always]{\frametitle{Can predict subgenome breeding values}
...
% \frame[label = current]{\frametitle{Two-Locus Epistasis}
...
\frame[label = always]{\frametitle{Subfunctionalization Epistasis}
\begin{columns}
...
\frame[label = always]{\frametitle{Homeologous interactions explain much of non-additive genetic signal}
...
\frame[label = always]{\frametitle{Summary of Homeoallelic Epistasis}
\begin{columns}
\begin{column}{0.15\linewidth}
  \includegraphics[page=12, clip, trim = 0in 0in 8.5in 0in, height=0.85\textheight]{"\string~/Dropbox/CUpres/armCartoon2"}
\end{column}
...
Breeding decisions based on subgenomes
 \begin{itemize}
...
\frame[label = current]{\frametitle{A traditional breeding program}
...
% \vspace{-1cm}
  ...
  \begin{scope}[xshift = 0cm, yshift = 0cm, scale=0.4]
    ...
    \begin{scope}[xshift = 0.1mm, yshift = -5cm, scale=1]
      \visible<5->{\node[label=below:``Big Red''] (pete) at (0,0) {\includegraphics[height = 1cm]{"\string~/Dropbox/wheatCartoons/alfalfa"}};}
    \end{scope}
...
% \frame{\frametitle{$21^\text{st}$ Century Plant Breeding Pipeline}
\frame[label = current]{\frametitle{Increasing population size increases intensity}
% High throughput phenotypes serve \highlite{2} purposes for breeding:
...
\frame[label = current]{\frametitle{Current model for genomic prediction works...}
  \begin{columns}
...
\frame[label = current]{\frametitle{... but can we do better?}
...
\frame[label = current]{\frametitle{Sparse testing is (almost) uniformly superior to unobserved prediction}
...
% \frame{\frametitle{$21^\text{st}$ Century Plant Breeding Pipeline}
\frame[label = current]{\frametitle{Rapid cycling greatly decreases $L$}
% High throughput phenotypes serve \highlite{2} purposes for breeding:
...
\frame[label = current]{\frametitle{Rapid cycle recurrent truncation beats traditional, at first}
\begin{columns}
...
\frame[label = current]{\frametitle{Parent selection must balance mean and variance}
\vspace{-1cm}
  \begin{columns}
    \begin{column}{0.5\linewidth}
      \centering
...
\frame[label = current]{\frametitle{Optimal contributions of parents}
% From Akdemir et al. (2019):
...
\visible<2>{
\begin{tikzpicture}
\node at (0, 0) {\includegraphics<2>[width = 0.65\linewidth, clip, trim = 1cm 1cm 1cm 1cm]{"\string~/Dropbox/optiBreedSim/figures/qp"}};%
...
\frame[label = current]{\frametitle{Optimal contributions can maintain $V_g$ at the cost of short-term gains}
\begin{columns}
...
\frame[label = current]{\frametitle{Can we have our cake and eat it too?}
\begin{columns}
\begin{column}{0.05\linewidth}
\end{column}
\begin{column}{0.45\linewidth}
...
\frame[label = current]{\frametitle{Rapid cycling }
...
\frame[label = current]{\frametitle{Alfalfa}
\begin{columns}
\begin{column}{0.5\linewidth}
\begin{itemize}
...
\frame[label = current]{\frametitle{US Alfalfa Farmer Research Initiative (USAFRI) Grant}
...
\frame[label = current]{\frametitle{Validation in diallel of 9 historic germplasm sources}
...
\frame[label = current]{\frametitle{Kinship in the diallel using AFLPs and GBS Fst}
    \vspace{-0.2cm}
    \begin{columns}
   \begin{column}{0.3\linewidth}
  Half-diallel
...
AFLPs track pedigree well
  ...
  \end{itemize}
    \end{column}
  \begin{column}{0.7\linewidth}
\includegraphics[width= \linewidth]{{{"\string /home/nsant/Dropbox/popLevelGenotyping/AvsKvsFst"}}}%
...
\frame[label = current]{\frametitle{Genomic prediction with AFLPs}
    \vspace{-0.2cm}
    \begin{columns}
   \begin{column}{0.3\linewidth}
...
\frame[label = current]{\frametitle{Recognition}
\begin{columns}
